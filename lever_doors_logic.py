            ...
            match element:
                ...
                case {'x': int() as x, 'y': int() as y,'action':'open-gate'}:
                    if   y < 0  or y > len(map_doors) or x > len(map_doors[0]) or  x < 0:
                       raise CustomException(f"door given at {(x, y)} is outside of the map")
                        ...
                        match switch:
                            case {'x': int() as x, 'y': int() as y}:
                                if  y < 0  or y > len(map_levers) or x > len(map_levers[0]) or  x < 0:
                                    raise CustomException(f"lever given at {(x, y)} is outside of the map")
                                ...
                                else: raise CustomException(f"There is no lever at (x, y) = {(x, y)}")
                            case _ :
                                raise CustomException("Please, use integer to precise the lever coordinate")
            match ymal_part:
